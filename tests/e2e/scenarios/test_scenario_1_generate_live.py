...
Run (against the local dev server):
    ...
    uv run pytest tests/e2e/scenarios/test_scenario_1_generate_live.py \\
        -v -s --no-cov
...
def scenario_base() -> str:
    ...
    for probe in ("/health", "/healthz"):  # local serves /health; deployed /healthz
        try:
            resp = httpx.get(f"{base}{probe}", timeout=15.0)
...
def scenario_page(browser_instance, scenario_base):
    ...
    Deliberately bypasses the int-workspace ``page`` fixture: this scenario
    talks to the *local* app and must not carry an int bearer token.
    """
